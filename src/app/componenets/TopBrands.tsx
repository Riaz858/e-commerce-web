...
//       {/* <div className="flex justify-between items-center gap-6 py-9"> */}
//       <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 py-9">
...
const TopBrands = () => {
  ...
  return (
<div className="py-8 px-4 sm:px-8 select-none">
  <div className="flex justify-between items-center">
    <h2 className="text-xl md:text-2xl font-bold">
      Top <span className="text-[#008fcc]">Electronic Brands</span>
    </h2>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 py-9">
    {brands.map((brand, index) => (
      <div
        key={index}
        className={`relative w-full h-64 p-4 rounded-xl shadow-lg ${brand.boxBgColor} overflow-hidden transition-transform duration-300 ease-in-out hover:scale-105`}
      >
        <div
          className={`absolute -top-28 -right-[68px] w-64 h-64 rounded-full ${brand.circleColor}`}
        ></div>
        <div
          className={`absolute -top-[130px] -right-[86px] border-[1.5px] w-72 h-72 rounded-full ${brand.circleLineColor}`}
        ></div>
        ...
        <img
          ...
          height={135}
          width={135}
          className="absolute right-4 bottom-4 object-contain hover:scale-105"
        />
        <p className={`text-lg font-semibold ${brand.save}`}>
          UP TO 80% OFF
        </p>
